...
function setup(app, options, cb) {
  if (options.setupDerby) {
    options.setupDerby(derby);
  }
  ...
  } else if (process.env.REDIS_URL || process.env.OPENREDIS_URL) {
    var redisUrl = parseUrl(process.env.REDIS_URL || process.env.OPENREDIS_URL);
    redisClient = redis.createClient(redisUrl.port, redisUrl.hostname);
    redisClient.auth(redisUrl.auth.split(":")[1]);
  } else {
    redisClient = redis.createClient();
  }
  redisClient.select(process.env.REDIS_DATABASE || 1);
  ...
  if (options.routes) {
    for (var i = 0; i < options.routes.length; i++) {
      ...
      expressApp[route[0]](route[1], route[2]);
    }
  }
